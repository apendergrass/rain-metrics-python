...
def calcrainmetrics(pdist,bincrates):
    ### we do this for rain amount or rain frequency dists
    tiles=0.1
    pdist[0]=0 # in case this is frequency, get rid of the dry frequency
    pmax=pdist.max()
    imax=np.nonzero(pdist==pmax)
    print 'peak: '+"{:.1f}".format(rainpeak)+' mm/d'
    print 'width: '+"{:.1f}".format(rainwidth)+' r2/r1'
    rainpeak=None
    rainwidth=None
    return rainpeak,rainwidth


pdist=pamt1
pdist=ppdf1

rmax=np.interp(1:length(bincrates),bincrates,imax);

clear thefrac
theps=(0.1:0.01:.99)*pmax;

for i=1:length(theps)
    thisp=theps(i);
    overp=(allyearpcont-thisp).*(allyearpcont> thisp);
    thefrac(i)=sum(overp)./sum(allyearpcont);
end
ptilerain=interp1(thefrac,theps,tiles);
overraintile=(allyearpcont-ptilerain).*(allyearpcont> ptilerain);
    r1=find(overraintile>0,1,'first')
    r2=find(overraintile>0,1,'last')

    (r2-r1)*0.0767+1
## this is the line for the rain amount width
    l=line([r1 r2],[1 1]*ptilerain);
### amount peak
pm=plot(imax,pmax,' pr')
set(pm,'markersize',10,'markerfacecolor','r')

### freq peak
allyearppdf=100./db*gpcpppdf1d;
pmax=max(allyearppdf(2:end))
imax=find(allyearppdf==pmax)

hold on;
pm=plot(imax,pmax,' pr')
set(pm,'markersize',10,'markerfacecolor','r')
#### end matlab code
    print 'rain amount peak: '+"{:.1f}".format(rainamtpeak)+' mm/d'
    print 'rain freq peak: '+"{:.1f}".format(rainpdfpeak)+' mm/d'
    print 'rain amount width: '+"{:.1f}".format(rainamtwidth)+' r2/r1'
    print 'rain freq width: '+"{:.1f}".format(rainpdfwidth)+' r2/r1'
    rainamtpeak=None
    rainpdfpeak=None
    rainamtwidth=None
    rainpdfwidth=None
    return rainamtpeak,rainpdfpeak,rainamtwidth,rainpdfwidth
...
makedistplots(ppdf1,pamt1,bincrates)
